...
public class PostRequest extends MethodBuilder{


    public PostRequest(String path, String token, @NonNull String[] bodyColumns, @NonNull String[] bodyData, @NonNull String[] headersColumns, @NonNull String[] headersData)
    {
        this.path = path;
        this.token = token;
    ...
    protected String execute() {
        ...
        if (bodyColumns[0].equals("")) {
                builder.method("POST", new FormBody.Builder().build());
            }
        // body exists
        else {
            ...
            builder.method("POST", requestJsonBody);
        }
        if (token.length() > 1){
